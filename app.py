# -*- coding: utf-8 -*-
"""
Professional monitoring dashboard for the crime/legal text classification model
(Crime & Legal Text Classification). Built with Streamlit + TensorFlow/Keras + Plotly.
"""
...
# ────────────────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Model Monitoring Dashboard | Crime Text AI",
    page_icon="⚖️",
    layout="wide",
...
# ────────────────────────────────────────────────────────────────────────────
# Modern, minimal styling (dark theme)
# ────────────────────────────────────────────────────────────────────────────
CUSTOM_CSS = """
<style>
...
# ────────────────────────────────────────────────────────────────────────────
# Load data, vectorizer, and models (cached)
# ────────────────────────────────────────────────────────────────────────────
...
def load_vectorizer():
    ...
    # The first two tokens (empty string and [UNK]) are reserved and added automatically
...
def predict_batch(model_name, texts_tuple):
    """Predict class probabilities for a batch of texts using a given model"""
...
def simulate_production_log(model_name, seed=7, n_events=1400, days_back=14):
    """
    Simulate a production traffic log by resampling from the full dataset and
    assigning random timestamps over the past N days — used to show throughput,
    alert history, and data drift trends over time.
    """
    ...
    # Weight business hours to make the throughput chart more realistic
    ...
    # Slight synthetic data drift over the last 2 days: slightly longer/shorter texts
...
with st.sidebar:
    ...
    st.caption("Crime/legal text classification model monitoring system")
    st.markdown("---")
    ...
    st.caption(f"Evaluated on the held-out test set — {len(test_df)} samples")
    ...
    st.caption("🟢 Service status: online")

# Simulated production log (for throughput / alerts / time trends)
...
st.markdown(
    f"""
    <div class="hero">
        <h1>Crime & Legal Text Classification — Model Monitoring Dashboard</h1>
        <p>Active model: <b>{model_name}</b> · Real-time monitoring of performance, data quality, and model behavior in production</p>
    </div>
    """,
...
# ════════════════════════════════════════════════════════════════════════════
# Tab 1 — Overview (Performance + Throughput)
# ════════════════════════════════════════════════════════════════════════════
with tabs[0]:
    st.markdown('<div class="section-title">📈 Model Performance Summary</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-sub">Key metrics on the test set + production traffic status</div>', unsafe_allow_html=True)
    ...
    k5.metric("Avg. Confidence (Production)", f"{avg_conf*100:.1f}%")
    ...
    with colA:
        ...
        st.markdown('<div class="section-title">⚡ Throughput — Predictions per Hour</div>', unsafe_allow_html=True)
        st.markdown('<div class="section-sub">Simulated production traffic over the last 14 days (by hour of day)</div>', unsafe_allow_html=True)
        ...
        fig = px.bar(
            ...
            labels={"x": "Hour of Day", "y": "Prediction Count"},
        ...
        st.caption(f"📦 Total logged predictions: **{total_events:,}** · Average throughput: **{avg_per_hour:.1f} predictions/hour**")
        st.markdown('</div>', unsafe_allow_html=True)

    with colB:
        st.markdown('<div class="glass-card">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">🏷️ Predicted Class Distribution</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="glass-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">🕓 Latest Logged Predictions</div>', unsafe_allow_html=True)
    recent = prod_log.sort_values("timestamp", ascending=False).head(8)[
        ["timestamp", "text", "pred_label", "confidence"]
    ].copy()
    recent["Predicted Class"] = recent["pred_label"].map(LABEL_NAMES)
    recent["Confidence"] = (recent["confidence"] * 100).round(1).astype(str) + "%"
    recent = recent.rename(columns={"timestamp": "Time", "text": "Text"})[
        ["Time", "Text", "Predicted Class", "Confidence"]
    ]
...
with tabs[1]:
    ...
    st.markdown('<div class="section-sub">Model performance on the test set, broken down by true and predicted class</div>', unsafe_allow_html=True)

    cm = confusion_matrix(test_eval["label"], test_eval["pred_label"], labels=CLASS_IDS)
    labels_en = [LABEL_NAMES[c] for c in CLASS_IDS]

    normalize = st.toggle("Normalize as percentage", value=False)
    if normalize:
    ...
    fig_cm = go.Figure(data=go.Heatmap(
        z=cm_show, x=labels_en, y=labels_en,
    ...
    with st.expander("📋 Detailed Per-Class Report"):
        rows = []
        for c in CLASS_IDS:
            ...
            rows.append({
                ...
                "Correct Prediction Rate": f"{(test_eval[mask]['correct'].mean()*100 if mask.sum() else 0):.1f}%",
            })
        st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)

# ════════════════════════════════════════════════════════════════════════════
# Tab 3 — Data Drift Detection + Over Time
# ════════════════════════════════════════════════════════════════════════════
with tabs[2]:
    st.markdown('<div class="section-title">🌊 Data Drift Detection</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-sub">Comparing the distribution of new input data (production) with the reference distribution (training data)</div>', unsafe_allow_html=True)
    ...
    d1.metric("Avg. Text Length (Reference)", f"{ref_len.mean():.1f} tokens")
    d2.metric("Avg. Text Length (Production)", f"{prod_len.mean():.1f} tokens")
    d3.metric("KS Statistic", f"{ks_stat:.3f}", delta=("⚠️ Significant Drift" if drift_flag else "✅ Stable"))

    colD1, colD2 = st.columns(2)
    with colD1:
        st.markdown('<div class="glass-card">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">Text Length Distribution: Reference vs Production</div>', unsafe_allow_html=True)
        fig_d = go.Figure()
        fig_d.add_trace(go.Histogram(x=ref_len, name="Reference Data (Train)", opacity=0.6,
                                      marker_color="#6366f1", histnorm="probability"))
        fig_d.add_trace(go.Histogram(x=prod_len, name="Production Data", opacity=0.6,
    ...
    with colD2:
        st.markdown('<div class="glass-card">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">Class Distribution: Reference vs Production</div>', unsafe_allow_html=True)
        ...
        fig_c = go.Figure()
        fig_c.add_trace(go.Bar(x=labels_en, y=ref_dist.values, name="Reference", marker_color="#6366f1"))
        fig_c.add_trace(go.Bar(x=labels_en, y=prod_dist.values, name="Production", marker_color="#ec4899"))
    ...
    st.markdown('<div class="glass-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">📉 Data Drift Trend Over Time</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-sub">Daily average token length of input texts vs. the reference baseline</div>', unsafe_allow_html=True)
    ...
    fig_t.add_trace(go.Scatter(
        ...
        name="Avg. Token Length (Daily)", line=dict(color="#f59e0b", width=3),
        marker=dict(size=7),
    ))
    fig_t.add_hline(y=ref_len.mean(), line_dash="dash", line_color="#6366f1",
                     annotation_text="Baseline (Reference)", annotation_position="top left")
    fig_t.update_layout(template=PLOTLY_TEMPLATE, height=340,
                         xaxis_title="Date", yaxis_title="Avg. Token Length",
                         margin=dict(l=10, r=10, t=20, b=10))
...
with tabs[3]:
    ...
    st.markdown('<div class="section-sub">Samples the model predicted with low confidence or incorrectly</div>', unsafe_allow_html=True)

    thresh = st.slider("Confidence threshold for labeling as 'low confidence'", 0.0, 1.0, 0.6, 0.05)

    low_conf = test_eval[test_eval["confidence"] < thresh].copy()
    wrong = test_eval[~test_eval["correct"]].copy()

    e1, e2, e3 = st.columns(3)
    e1.metric("Low-Confidence Samples", len(low_conf))
    e2.metric("Incorrect Predictions", len(wrong))
    e3.metric("Overall Error Rate", f"{(1-acc)*100:.1f}%")

    view = st.radio("Show:", ["Low Confidence", "Incorrect Predictions", "Both"], horizontal=True)
    if view == "Low Confidence":
        show_df = low_conf
    elif view == "Incorrect Predictions":
        show_df = wrong
    else:
        show_df = test_eval[(test_eval["confidence"] < thresh) | (~test_eval["correct"])]

    show_df = show_df.sort_values("confidence").copy()
    show_df["True Label"] = show_df["label"].map(LABEL_NAMES)
    show_df["Predicted Label"] = show_df["pred_label"].map(LABEL_NAMES)
    show_df["Confidence"] = (show_df["confidence"] * 100).round(1).astype(str) + "%"
    st.dataframe(
        show_df.rename(columns={"text": "Text"})[["Text", "True Label", "Predicted Label", "Confidence"]],
        use_container_width=True, hide_index=True, height=420,
    )

    st.markdown('<div class="glass-card">', unsafe_allow_html=True)
    st.markdown('<div class="section-title">Model Confidence by Prediction Correctness</div>', unsafe_allow_html=True)
    fig_e = px.box(
        test_eval, x="correct", y="confidence", color="correct",
        color_discrete_map={True: "#22c55e", False: "#ef4444"},
        labels={"correct": "Was Prediction Correct?", "confidence": "Model Confidence"},
...
with tabs[4]:
    ...
    st.markdown('<div class="section-sub">Automatic alerts generated based on accuracy drops, low confidence, or data drift within daily windows</div>', unsafe_allow_html=True)
    ...
    for date, g in daily_log.groupby("date"):
        ...
        if day_acc < 0.55:
            alerts.append((date, "Accuracy Drop", f"Daily accuracy dropped to {day_acc*100:.1f}%", "Red"))
        elif day_acc < 0.68:
            alerts.append((date, "Accuracy Drop", f"Daily accuracy dropped to {day_acc*100:.1f}%", "Yellow"))

        if day_conf < 0.55:
            alerts.append((date, "Low Confidence", f"Average model confidence was {day_conf*100:.1f}%", "Red"))
        elif day_conf < 0.65:
            alerts.append((date, "Low Confidence", f"Average model confidence was {day_conf*100:.1f}%", "Yellow"))

        if drift_p < 0.01:
            alerts.append((date, "Data Drift", f"Significant change in input text length (KS={drift_stat:.2f})", "Red"))
        elif drift_p < 0.05:
            alerts.append((date, "Data Drift", f"Minor change in input data distribution (KS={drift_stat:.2f})", "Yellow"))

        if len(g) > daily_log.groupby("date").size().mean() * 1.6:
            alerts.append((date, "Traffic Spike", f"{len(g)} requests logged on this day (above normal)", "Yellow"))

    alerts_df = pd.DataFrame(alerts, columns=["Date", "Alert Type", "Message", "Severity"])
    alerts_df = alerts_df.sort_values("Date", ascending=False)

    if alerts_df.empty:
        st.success("✅ No alerts logged in the recent period — model performance has been stable.")
    else:
        n_red = (alerts_df["Severity"] == "Red").sum()
        n_yellow = (alerts_df["Severity"] == "Yellow").sum()
        a1, a2, a3 = st.columns(3)
        a1.metric("Total Alerts", len(alerts_df))
        a2.metric("Critical Alerts 🔴", int(n_red))
        a3.metric("Warning Alerts 🟡", int(n_yellow))

        severity_filter = st.multiselect("Filter by severity", ["Red", "Yellow"], default=["Red", "Yellow"])
        filtered_alerts = alerts_df[alerts_df["Severity"].isin(severity_filter)]

        for _, row in filtered_alerts.iterrows():
            badge_class = "badge-red" if row["Severity"] == "Red" else "badge-yellow"
            st.markdown(
                f"""
                <div class="glass-card" style="margin-bottom:8px;">
                    <span class="badge {badge_class}">{row['Severity']}</span>
                    <b style="margin-left:8px;">{row['Alert Type']}</b>
                    <span style="color:rgba(255,255,255,0.5); font-size:0.85rem; float:right;">{row['Date']}</span>
                    <div style="margin-top:6px; color:rgba(255,255,255,0.85);">{row['Message']}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
...
# ════════════════════════════════════════════════════════════════════════════
# Tab 6 — Filter, Search & Export
# ════════════════════════════════════════════════════════════════════════════
with tabs[5]:
    st.markdown('<div class="section-title">🗂️ Filter, Search & Export</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-sub">Explore the simulated production prediction log and export results</div>', unsafe_allow_html=True)
    ...
    st.caption(f"🔢 Results: **{len(filtered):,}** of {len(prod_log):,} records")

    display_df = filtered.copy()
    display_df["True Label"] = display_df["label"].map(LABEL_NAMES)
    display_df["Predicted Class"] = display_df["pred_label"].map(LABEL_NAMES)
    display_df["Confidence (%)"] = (display_df["confidence"] * 100).round(1)
    display_df = display_df.rename(columns={"timestamp": "Time", "text": "Text"})
    st.dataframe(
        display_df[["Time", "Text", "True Label", "Predicted Class", "Confidence (%)"]]
    ...
    with exp1:
        ...
        st.download_button(
            "⬇️ Download Full CSV (Filtered Data)",
            data=csv_bytes,
    ...
    with exp2:
        report_lines = [
            ...
            f"Report date: {datetime(2026, 8, 30).strftime('%Y-%m-%d')}",
            "",
            ...
            f"Logged records: {len(prod_log)}",
            f"Average confidence: {prod_log['confidence'].mean()*100:.2f}%",
            f"Average text length (production): {prod_log['token_len'].mean():.2f} tokens",
            f"Average text length (reference): {ref_len.mean():.2f} tokens",
            f"Data drift KS statistic: {ks_stat:.3f} (p-value={ks_p:.4f})",
            "",
            "-- Errors --",
            f"Low-confidence samples (<0.6): {(prod_log['confidence'] < 0.6).sum()}",
        ]
        report_text = "\n".join(report_lines)
        st.download_button(
...
# ════════════════════════════════════════════════════════════════════════════
with tabs[6]:
    st.markdown('<div class="section-title">💡 Prediction Explainability</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-sub">By temporarily removing each word (occlusion), its impact on the model\'s decision is measured and displayed</div>', unsafe_allow_html=True)

    mode = st.radio("Text source:", ["Choose from test samples", "Write custom text"], horizontal=True)
    if mode == "Choose from test samples":
        ...
        input_text = st.text_area("Enter text (in English, matching the training data):",
    ...
    if st.button("🔮 Run Explainability Analysis", type="primary"):
        ...
        for i in range(n_show):
            ...
            modified[i] = 0  # Temporarily remove the token (replace with padding)
        ...
        r1.metric("Predicted Class", LABEL_NAMES[pred_class])
        r2.metric("Model Confidence", f"{base_conf*100:.1f}%")
        if true_label is not None:
            r3.metric("True Label", LABEL_NAMES[int(true_label)])
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown('<div class="glass-card">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">🖍️ Highlighting Words That Influenced the Decision</div>', unsafe_allow_html=True)
        st.caption("Darker color = greater word impact on the model's confidence in the predicted class")
        ...
        st.markdown(
            f'<div style="text-align:left; line-height:2.4; font-size:1.05rem;">{" ".join(html_words)}</div>',
            unsafe_allow_html=True,
        )
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown('<div class="glass-card">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">📊 Word Importance Score</div>', unsafe_allow_html=True)
        imp_df = pd.DataFrame({"Word": words[:n_show], "Importance Score": importances})
        imp_df = imp_df.sort_values("Importance Score", ascending=True)
        fig_imp = px.bar(
            imp_df, x="Importance Score", y="Word", orientation="h",
            template=PLOTLY_TEMPLATE, color="Importance Score",
        ...
        st.markdown('<div class="glass-card">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">📈 Probability Distribution Across All Classes</div>', unsafe_allow_html=True)
        prob_df = pd.DataFrame({
            "Class": [LABEL_NAMES[c] for c in CLASS_IDS],
...
st.markdown(
    ...
    <div style="text-align:center; color:rgba(255,255,255,0.35); font-size:0.8rem; margin-top:30px;">
        Model Monitoring Dashboard · Built with Streamlit, TensorFlow/Keras, and Plotly
    </div>
    """,
    unsafe_allow_html=True,
)
